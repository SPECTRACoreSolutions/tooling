...
def _ensure_utf8_stdio() -> None:
    """Avoid UnicodeEncodeError when printing emoji on Windows (cp1252) consoles."""
...
try:
    ...
    # stdout: optional PC RGB only — must not go to stderr or Elysia surfaces this as the sole "error"
    print("ℹ️  OpenRGB Python SDK not installed (optional — Hue/Nanoleaf unaffected)")
...
try:
    ...
    print("⚠️  httpx not installed (required for Hue/Nanoleaf — pip install httpx)")
...
class UnifiedLightController:
    ...
    def set_all_blue(self, red=0, green=100, blue=255):
        """Set all configured lighting backends to the given RGB. Returns True if at least one succeeded."""
        print(f"\n🎨 Setting lights to RGB({red}, {green}, {blue})\n")
        ...
        if any(results.values()):
            print("🎉 At least one lighting system applied that colour.\n")
        else:
            print(
                "⚠️  No lighting system accepted the command — check Hue bridge IP/username in "
                "lighting-config.json, Nanoleaf token, OpenRGB + SDK, and pip install httpx in this Python.\n"
            )
    ...
    def _set_openrgb_blue(self, red, green, blue):
        ...
        try:
            ...
            print(f"   ✅ Set {device_count} OpenRGB device(s) to the target colour")
    ...
    def _set_hue_blue(self, red, green, blue):
        ...
        try:
            ...
            # Get all lights
            url = f"https://{self.hue_bridge_ip}/api/{self.hue_username}/lights"
            response = httpx.get(url, timeout=5.0, verify=False)
            ...
            for light_id in lights.keys():
                light_url = f"https://{self.hue_bridge_ip}/api/{self.hue_username}/lights/{light_id}/state"
                ...
                light_url = f"https://{self.hue_bridge_ip}/api/{self.hue_username}/lights/{light_id}/state"
                result = httpx.put(light_url, json=payload, timeout=5.0, verify=False)
                if result.status_code == 200:
                    light_count += 1

            print(f"   ✅ Set {light_count} Hue light(s) to the target colour")
            return light_count > 0
    ...
    def _set_nanoleaf_blue(self, red, green, blue):
        ...
        try:
            ...
            print("   ✅ Set Nanoleaf panels (static effect applied)")
            return True
...
def main():
    """Main entry point — exit code 0 only if at least one backend applied the colour."""
    ...
    if len(sys.argv) > 1:
        if sys.argv[1] in ("--help", "-h", "help"):
            show_help()
            return
        if sys.argv[1] == "--color" and len(sys.argv) == 5:
            try:
                red = int(sys.argv[2])
    ...
    ok = controller.set_all_blue(red, green, blue)
